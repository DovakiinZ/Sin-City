...
interface MinimalEditorProps {
    ...
    setIsRegisteredOnly: (v: boolean) => void;
        ...
        <div className="space-y-4">
            ...
            <div className={`relative bg-black/40 border rounded-xl transition-all duration-300 ${isFocused
                ...
                <div className="absolute bottom-0 left-0 right-0 flex items-center justify-between px-4 py-3 border-t border-green-900/20 bg-black/30 rounded-b-xl">
                    ...
                    {/* Character count or Visibility Toggle */}
...
export default function CreatePost() {
    ...
    // Start behavior tracking and fetch site settings for guests
    useEffect(() => {
        if (!user) {
            startTracking();
            ...
            const checkSetting = async () => {
                ...
                if (data) {
                    const allowed = data.value === true || data.value === 'true';
                    setAllowAnonPosts(allowed);
                    if (!allowed) {
                        toast({
                            title: "Access Denied",
                            description: "Anonymous posting is currently disabled.",
                            variant: "destructive"
                        });
                        navigate('/');
                    }
    ...
    }, [user, startTracking, navigate, toast]);
