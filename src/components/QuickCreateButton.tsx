...
export default function QuickCreateButton() {
    ...
    useEffect(() => {
        const fetchSetting = async () => {
            try {
                ...
                if (data) {
                    setAllowAnonPosts(data.value === true || data.value === 'true');
                } else {
                    setAllowAnonPosts(true);
                }
            } catch (error) {
                console.error("[QuickCreateButton] Error fetching setting:", error);
                setAllowAnonPosts(true);
            }
        };
    ...
    // Only show if user is logged in OR (it's NOT loading AND anon posts are allowed)
    const shouldShow = user || (allowAnonPosts === true);
