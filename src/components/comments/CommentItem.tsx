...
export default function CommentItem({ comment, postId, postAuthorId, depth = 0 }: CommentItemProps) {
    ...
    const handleToggleHide = async () => {
        try {
            await updateComment(comment.id, { hidden: !comment.hidden });
            toast({ 
                title: comment.hidden ? "Unhidden" : "Hidden", 
                description: `Comment is now ${comment.hidden ? "visible" : "hidden"}` 
            });
        } catch (error) {
        ...
        <div className={`py-3 group ${depth > 0 
            ...
            <div className={`flex items-center gap-2 mb-2 ${contentIsArabic ? 'flex-row-reverse' : ''}`}>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    {/* Actions Menu */}
                    {((isOwner && !isEditing) || user?.role === 'admin') && (
                        <DropdownMenu>
            ...
            {/* Comment Content */}
            <div className={`pl-9 ${comment.hidden ? 'opacity-50' : ''}`}>
                {isEditing ? (
