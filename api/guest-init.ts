...
export default async function handler(
    ...
    try {
        ...
        // Call ipapi.co for geolocation (SSL supported, more reliable on Vercel)
        ...
        if (geoData.error) {
            // Return basic data if geo lookup fails or limit reached
            const clientResult: ClientGeoData = {
                ip_hash: hashIP(ip),
                ip_source: source,
                country: 'Unknown',
            ...
            return res.status(200).json({
                ...
                ip_encrypted: realIp // Pass plain IP back as requested for accuracy
            });
        }
        ...
        const clientResult: ClientGeoData = {
            ...
            vpn_detected: detectVPN(geoData.org || '', geoData.org || '', geoData.asn || ''),
            tor_detected: detectTor(geoData.org || '', geoData.org || ''),
        };
        ...
        return res.status(200).json({
            ...
            ip_encrypted: realIp // Pass plain IP back for accuracy (legacy field used by hook)
        });
